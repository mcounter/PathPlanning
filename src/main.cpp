...
int main() {
	...
	// Number of lanes on the drivable side of the road.
	int lane_num = 3;

	// Speed limit in MPH
	double speed_limit = 50;

	//Time difference between path points in seconds
	double refresh_time = 0.02;
	...
	PathPlanner planner;
	planner.init(map_waypoints_x, map_waypoints_y, map_waypoints_s, map_waypoints_dx, map_waypoints_dy,
		max_s, lane_width, lane_num, refresh_time);

#if _NEW_UWS_VERSION
	h.onMessage([&planner, &map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy, &speed_limit](uWS::WebSocket<uWS::SERVER> *ws, char *data, size_t length,
		uWS::OpCode opCode) {
